...
USAGE = """fuaran-py — the Fuaran Python host CLI

Usage:
  fuaran-py validate <file> [--kind node|op] [--json]   Wire JSON -> pass/fail + diagnostics
  fuaran-py render <file>                               Wire JSON -> server-HTML body fragment
  fuaran-py export <file> --format markdown|email|email-document [--title T] [--subject S]
  fuaran-py corpus-sync [--check | --declare] [<path>]  Bundled corpus snapshot vs the authority
  fuaran-py help                                        This text
...
def _validate(argv: list[str]) -> CliResult:
    ...
    commentary = [f"fuaran-py validate: posture={POSTURE} - {POSTURE_NOTE}"]
    ...
    if findings:
        commentary.append(
            f"fuaran-py validate: {len(findings)} structural finding(s) - advisory; the exit code "
...
def _render(argv: list[str]) -> CliResult:
    ...
    return CliResult(0, html + "\n", f"fuaran-py render: {len(html)} byte(s) of body-fragment HTML\n")
...
def _export(argv: list[str]) -> CliResult:
    ...
    return CliResult(0, document, f"fuaran-py export: {fmt}, {len(document)} byte(s)\n")


def _sync_corpus_script() -> Path | None:
    """Locate ``conformance/sync_corpus.py`` — a repo-checkout artefact.

    It is not part of the installed package (the wheel ships ``src/fuaran_py``
...
def _corpus_sync(argv: list[str]) -> CliResult:
    ...
    spec = importlib.util.spec_from_file_location("fuaran_py_cli_sync_corpus", script)
...
def main(argv: list[str] | None = None) -> int:
    """Console-script entry point (``fuaran-py``). Returns the process exit code."""
