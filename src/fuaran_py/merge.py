...
to, except the closure-free style sub-fields, compared directly. The merge is
**author-agnostic** — both sides are treated equally; human-pin primacy is an
orchestration-tier concern, not a wire-host one.
"""
...
@dataclass(frozen=True)
class MergeOk:
    """A fully auto-merged result carrying the merged tree."""
...
class MergeConflicts:
    """A merge that surfaced one or more conflicting cells (trunk unchanged)."""
...
def _pick_field(
    ...
    if a_ch and b_ch and a_v != b_v:
        conflicts.append(MergeConflict(node_id, facet))
        return base_v
    if a_ch:
        return a_v
    if b_ch:
...
def _merge_style(conflicts: list[MergeConflict], node_id: str, base: Node, a: Node, b: Node) -> Obj | None:
    tone = _pick_field(
        conflicts, node_id, "style.tone", _style_field(base, "tone"), _style_field(a, "tone"), _style_field(b, "tone")
    )
    weight = _pick_field(
        conflicts,
        node_id,
    ...
    role = _pick_field(
        conflicts, node_id, "style.role", _style_field(base, "role"), _style_field(a, "role"), _style_field(b, "role")
    )
    voice = _pick_field(
        conflicts,
        node_id,
...
def _merge3(conflicts: list[MergeConflict], base: Node, a_opt: Node | None, b_opt: Node | None) -> Node:
    ...
    kind_pick = _pick_canonical(
        conflicts, node_id, "kind", _kind_canonical(base), _kind_canonical(a), _kind_canonical(b)
    )
    kind_source = a if kind_pick == 1 else b if kind_pick == 2 else base

    # style sub-fields (independent)
    merged_style = _merge_style(conflicts, node_id, base, a, b)
    ...
    def recurse_child(cid: str) -> Node:
        bc = base_map.get(cid)
        if bc is not None:
            return _merge3(conflicts, bc, a_map.get(cid), b_map.get(cid))
        ac = a_map.get(cid)
        if ac is not None:
            return ac
        bb = b_map.get(cid)
    ...
    if not a_struct and not b_struct:
        ...
        if disjoint:
            ...
            conflicts.append(MergeConflict(node_id, "children"))
            merged_children = [recurse_child(i) for i in base_ids]
...
def merge_3way(base: Node, a: Node, b: Node) -> MergeResult:
    ...
    """
    conflicts: list[MergeConflict] = []
    merged = _merge3(conflicts, base, a, b)
