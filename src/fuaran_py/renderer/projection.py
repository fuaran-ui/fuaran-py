...
markdown **document** (:mod:`fuaran_py.renderer.document`) and an email-safe **digest**
(:mod:`fuaran_py.renderer.email`) — render into targets with no runtime at all, and both
...
def behavioural_wire_kinds(manifest: Mapping[str, object]) -> list[str]:
    ...
    corpus is, for the same reason :mod:`fuaran_py.render_fidelity` does not.
    """
