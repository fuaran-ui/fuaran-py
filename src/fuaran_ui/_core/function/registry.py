...
**Reuse.** A hole's value envelope is the same closed :class:`~fuaran_ui._core.function.space.HoleSpace`
vocabulary (``intRange`` / ``floatRange`` / ``stringLen`` / ``enum`` / ``anyString``) the
capability registry already ships — it *is* the F# ``ValueSpace``, so it lives beside this
registry inside the Core boundary and the capability registry re-exports it. The function
