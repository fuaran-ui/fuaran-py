...
:func:`~fuaran_ui.op_stream.hash_chain.verify_chain` for that. The two concerns
...
:class:`~fuaran_ui.op_stream.types.CasOpStreamSink` extension,
...
def apply_to(initial_tree: Node, records: list[OpRecord], *, include_refused: bool = False) -> ReplayResult:
    ...
    **A record whose ``result_envelope`` is not :class:`~fuaran_ui.op_stream.types.Success`
