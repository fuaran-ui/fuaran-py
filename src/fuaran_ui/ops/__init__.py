"""``fuaran_ui.ops`` — the ``TreeOp`` codec.

Public surface:

* :func:`~fuaran_ui.ops.decode.decode_op` — wire JSON → ``Result[Obj, DecodeError]``
* :func:`~fuaran_ui.ops.encode.encode_op` — decoded op → canonical wire JSON
* :mod:`fuaran_ui.ops.placement` — the placement algebra (``place_op`` / ``move_op`` /
