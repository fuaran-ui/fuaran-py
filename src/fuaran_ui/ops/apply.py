...
tree, returning either the new tree (:class:`~fuaran_ui.result.Ok`) or a typed,
...
The engine folds over the **generic structural model** (:mod:`fuaran_ui.model`) the
...
@dataclass(frozen=True)
class ApplyErr:
    """A failed apply carrying the :class:`ApplyError` (mirrors :class:`~fuaran_ui.result.Err`)."""
...
def apply(op: Obj, tree: Node) -> ApplyResult:
    ...
    Returns :class:`~fuaran_ui.result.Ok` carrying the new tree, or
