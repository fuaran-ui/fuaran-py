...
client hydrates; :mod:`fuaran_ui.renderer.email` produces a digest an inbox survives; this
...
:func:`~fuaran_ui.renderer.bindings.render_text` and figures through
:func:`~fuaran_ui.renderer.bindings.format_number` — the same functions the HTML page and
...
# Package-internal reuse, for the reason stated in :mod:`fuaran_ui.renderer.email`: a second
...
def render_markdown(
    ...
    *,
    ...
    :param sources: the host binding map, resolved exactly as
        :func:`~fuaran_ui.renderer.render_html` resolves it — including the state seeds the
