...
``text/html``
    the shipped server renderer's body fragment (:func:`fuaran_ui.renderer.render_html`)
...
``application/vnd.fuaran.ui+json``
    the canonical wire JSON, as the **string** :func:`fuaran_ui.encode_node`
...
:func:`~fuaran_ui.renderer.render_html` returns, byte for byte, so every ``href``
...
def display_html(
    ...
    The fragment is :func:`~fuaran_ui.renderer.render_html`'s output unchanged —
