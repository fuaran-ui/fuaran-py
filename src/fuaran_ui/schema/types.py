...
The codec floor (:mod:`fuaran_ui.model`) is a deliberately *generic*
...
* **Authoring** uses these typed dataclasses; every one **lowers** to the generic
  :class:`~fuaran_ui.model.Node` / :class:`~fuaran_ui.model.Obj` via :func:`_lower`,
  and the proven canonical encoder (:func:`fuaran_ui.canonical.encode_value`) does the
...
defaults + ARIA injection) live in :mod:`fuaran_ui.ui`; this module is the data.
"""
...
class _WireConvertible(Protocol):
    """Anything that lowers to a canonical wire :data:`~fuaran_ui.model.Value`."""
...
def _lower(value: object) -> Value:
    ...
    as in :func:`fuaran_ui.model.from_json`); typed dataclasses defer to their
    ``to_wire``; sequences become :class:`~fuaran_ui.model.Arr`; a plain ``dict``
    becomes a tag-less :class:`~fuaran_ui.model.Obj` (a ``JsonValue`` record).
    """
...
def _obj(tag: str | None, fields: dict[str, object]) -> Obj:
    """Build a structural :class:`~fuaran_ui.model.Obj`, dropping ``None`` fields.
...
class InvokeArg:
    ...
    type lives (``fuaran_ui.ui.capability.HoleSpace``), and it validates the
...
class Invoke:
    ...
    :class:`~fuaran_ui.ui.capability.CapabilityRegistry`, and the SHAPE is refused
...
# :mod:`fuaran_ui.charts`. That split is why the union is a WIRE vocabulary and
...
class UiNode:
    ...
    Lowers to the generic :class:`~fuaran_ui.model.Node` via :meth:`to_wire`; the
    ...
    def _repr_mimebundle_(
        ...
        what :func:`~fuaran_ui.schema.encode.encode_node` and the renderer already
        consume — one display path, not a second one that could drift from it.
        :mod:`fuaran_ui.renderer.notebook` carries the contract.
        """
