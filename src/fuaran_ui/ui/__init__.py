"""``fuaran_ui.ui`` — the ergonomic, typed authoring surface.

The Python analogue of ``@fuaran-ui/ui`` / ``Fuaran.UI``: smart constructors over
the typed per-kind model (:mod:`fuaran_ui.schema.types`) that inject per-kind
...
def _switch_case(entry: SwitchCaseInput) -> t.SwitchCase:
    """Coerce one ``fuaran.switch`` case entry to a :class:`~fuaran_ui.schema.types.SwitchCase`.
...
class binding:  # noqa: N801 — namespace object, mirrors the cross-tier `binding.*` vocabulary
    ...
    def invoke(capability_id: str, **args: str) -> Binding:
        ...
        the wire (see :data:`~fuaran_ui.ui.capability.InvokeArgValue`).
        """
...
class fuaran:  # noqa: N801 — namespace object, mirrors the cross-tier `fuaran.*` vocabulary
    ...
    def drawing(
        ...
        *,
        ...
        :class:`~fuaran_ui.schema.types.DrawStyle`.
    ...
    def switch(  # noqa: A003
        ...
        *,
        ...
        A case is a ``(selector, child)`` pair or a :class:`~fuaran_ui.schema.types.SwitchCase`
    ...
    def mount(
        ...
        *,
        ...
        ``inputs`` shares :data:`~fuaran_ui.schema.types.FragmentArg` with
...
def encode(n: UiNode) -> str:
    ...
    canonical encoder — byte-identical to :func:`fuaran_ui.encode_node` over the
...
# host-registration seam. The wire codec lives in :mod:`fuaran_ui.dataframe`.
