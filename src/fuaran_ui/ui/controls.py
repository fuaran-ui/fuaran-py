"""``fuaran_ui.ui.controls`` — controls whose slot feeds a Compute parameter.

:mod:`fuaran_ui.ui.quick` makes a dashboard out of the data a notebook already holds.
This makes that dashboard **answer back**: a control declares a state slot, a pipeline
reads that slot through :func:`~fuaran_ui.ui.compute.param`, and a host with no Python
...
is what makes a name resolvable, and :meth:`~fuaran_ui.ui.compute.Frame.to_transform_binding`
...
class Control(UiNode):
    ...
    It IS a :class:`~fuaran_ui.schema.types.UiNode` — drop it straight into a dashboard's
...
def _as_binding(transform: TransformBinding) -> Binding:
    ...
    a fudge — the same one :mod:`fuaran_ui.ui.quick` makes for a node's ``source``."""
