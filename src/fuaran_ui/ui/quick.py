"""``fuaran_ui.ui.quick`` — the terse, notebook-grade authoring layer.

:mod:`fuaran_ui.ui` is **id-first**: every constructor's first positional argument is
...
per-kind constructors in :mod:`fuaran_ui.ui` — it is a layer *over* that surface,
...
returns is an ordinary :class:`~fuaran_ui.schema.types.UiNode`; mix the two surfaces
...
* **Same input → same ids.** The derivation reads nothing but its arguments — no
  ...
  a byte-identical tree, and :func:`fuaran_ui.ops.diff` yields the empty op script.
...
def _children_of(node: UiNode) -> tuple[UiNode, ...] | None:
    ...
    Only :class:`~fuaran_ui.schema.types.Box` is walked, because ``dashboard`` and
...
def _frame_binding(records: Sequence[Mapping[str, object]], keys: Sequence[str], *, with_row_key: bool) -> Binding:
    ...
    Built through :func:`fuaran_ui.ui.frame`, so the columnar encoding, the type
    ...
    :mod:`fuaran_ui.schema.types`, and ``TransformBinding`` — the seventh wire case —
    lives a layer above it in :mod:`fuaran_ui.ui.compute`, because it needs the
